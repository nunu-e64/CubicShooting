...
void GameAwake(){		//ÉQÅ[ÉÄãNìÆéûèàóù
...
bool GameLoop(){
	...
	//îwåiï`âÊÇ∆ÉXÉ^Å[ÉåÅ[É_Å[ÇÃï`âÊ
	Stage.Draw(Mine.GetPos());
	
	//êØà⁄ìÆÇ∆ï`âÊÇ∆ç≈Ç‡ãﬂÇ¢êØà íuÇÃîªíË
	Vector nearest_star;
...
bool Choice(){
	...
	while( ScreenFlip()==0 && ProcessMessage()==0 && ClearDrawScreen()==0 ){
		...
		DrawFormatString(WINDOW_WIDTH/2 - 60, 100, WHITE,"Å©Å@STAGE %dÅ@Å®", Stage.GetStageNum()) ;
		DrawCenterString(WINDOW_WIDTH/2, 180, "Å™", WHITE);
		if (Stage.GetDifficulity()){
			DrawCenterString(WINDOW_WIDTH/2, 210, "HARD", WHITE);
		}else{
			DrawCenterString(WINDOW_WIDTH/2, 210, "EASY", WHITE);		
		}
		DrawCenterString(WINDOW_WIDTH/2, 240, "Å´", WHITE);
